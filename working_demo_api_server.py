#!/usr/bin/env python3
"""
Working Demo API server for the Flight Scheduler Dashboard
This provides comprehensive mock data with all the enhanced features.
"""
...
app = FastAPI(
    title="Agentic Flight Scheduler API - Working Demo",
    description="Working demo API for flight scheduling dashboard",
    version="1.0.0"
)
...
def initialize_demo_data():
    ...
    global demo_flights
    flight_ids = [f"AI{2700 + i}" for i in range(20)] + [f"6E{100 + i}" for i in range(10)]
    
    for flight_id in flight_ids:
        demo_flights[flight_id] = {
            "flight_id": flight_id,
            ...
            "origin": random.choice(["BOM", "DEL", "BLR", "MAA"]),
            "destination": random.choice(["BOM", "DEL", "BLR", "MAA"])
        }
...
def generate_time_buckets(bucket_minutes: int = 10):
    """Generate realistic time bucket data"""
    buckets = []
...
async def root():
    return {
        "message": "Agentic Flight Scheduler API - Working Demo",
        "version": "1.0.0",
        "status": "ready",
        "features": ["analytics", "optimization", "delay_prediction", "whatif", "alerts"]
    }
...
async def whatif_analysis(request: Dict[str, Any]):
    ...
    if change_type == "time_shift":
        try:
            delay_change = float(change_value.replace('+', '').replace('-', '').replace('m', ''))
...
async def get_delay_risks(
    ...
    """Enhanced delay risk prediction endpoint"""
    risks = []
...
if __name__ == "__main__":
    print("🚀 Starting Agentic Flight Scheduler Working Demo API...")
